...
const Schedules = (props) => {
  ...
  const Delete = (id) => {
    if (window.confirm("برای حذف این زمانبندی مطمئن هستید؟")) {
      deleteRequest("/schedules/" + id)
        .then((result) => {
          toast.success("زمانبندی مورد نظر حذف شد.");
          getData();
        })
        .catch((error) => {
  ...
  const search = (f, v) => {
    searchParams["TblSchedulesSearch[" + f + "]"] = v;
    getData();
  };
  ...
  return (
    <>
      <Row>
        <Col>
          <Button as={Link} to="/schedules/create">
            ایجاد زمانبندی
          </Button>
        </Col>
      </Row>
